...
public class GithubPortfolioServiceImpl implements GithubPortfolioService {
    ...
    /** Tried in order; the first one with content becomes the extra context. */
    private static final List<String> BUILD_FILES = List.of("package.json", "pom.xml", "build.gradle");
    ...
    public PortfolioResponse.PortfolioProjectResponse importFromGithub(GithubImportRequest request) {
        ...
        PortfolioResponse.PortfolioProjectResponse project = analyzeRepo(request.getRepoUrl(), student, catalog);
    ...
    private PortfolioResponse.PortfolioProjectResponse buildProject(
    ...
    public GithubImportAuditResponse getImportAudit(String repoUrl) {
        ...
        Map<String, EvidenceStatus> statusBySkill = skillEvidenceStatuses(student.getUserId(), repoUrl);

        List<SkillMatch> matches = audit.getMatchedSkills() == null ? List.of() : audit.getMatchedSkills();
        List<GithubImportAuditResponse.MatchedSkillResponse> skills = matches.stream()
                .map(match -> GithubImportAuditResponse.MatchedSkillResponse.builder()
                        .skill(match.skill())
        ...
        return GithubImportAuditResponse.builder()
                ...
                .sources(sources.stream()
                        .map(source -> GithubImportAuditResponse.SourceReadResponse.builder()
                                .path(source.path())
                                .chars(source.chars())
                                .found(source.found())
                                .build())
    ...
    private RepoEvidenceResponse describeEvidence(UUID userId, String repoUrl) {
        ...
        List<RepoEvidenceResponse.EvidenceSkillResponse> skills = statuses.entrySet().stream()
                .map(entry -> RepoEvidenceResponse.EvidenceSkillResponse.builder()
                        .skill(displayNames.getOrDefault(entry.getKey(), entry.getKey()))
                        .status(entry.getValue().name())
                        .build())
                .sorted(Comparator.comparing(RepoEvidenceResponse.EvidenceSkillResponse::getSkill))
                .toList();
